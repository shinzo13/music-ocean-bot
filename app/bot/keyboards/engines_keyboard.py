...
def engines_keyboard(engine: Engine):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[  # noqa
        ...
        [InlineKeyboardButton(
            text=option_selection("Yandex Music", engine == Engine.YANDEX),
            callback_data=DefaultEngineCallback(engine_prefix='ya').pack(),
            icon_custom_emoji_id=YANDEX_EMOJI_ID,
            style='success' if engine == Engine.YANDEX else None
        )],
        [InlineKeyboardButton(
